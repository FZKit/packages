    ...
    return fp<U>(
      async (scope, options) => {
        try {
          ...
          scope.log.error(error, `Error registering "${this.constructor.name}"`);
          throw error;
        }
...
export function createFastifyPlugin<T extends FastifyInstance, U extends FastifyPluginOptions>(
  Plugin: new () => FZKitPlugin<T, U>,
): FastifyPluginCallback<U> {
